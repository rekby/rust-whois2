...
fn decide_test() {
    ...
    assert_eq!(
        decide(domain, &whois, &vec!()).unwrap(),
        Decision::NextWhois(NextWhois {
            domain: "ru",
            whois_server: "whois.root"
        })
    );
...
fn get_whois_server_test() {
    let c = Client::new();
    assert_eq!(
        c.get_whois_server("").unwrap(),
        ROOT_WHOIS_SERVER.to_owned()
    );
    assert_eq!(
        c.get_whois_server("ru").unwrap(),
        ROOT_WHOIS_SERVER.to_owned()
    );
    assert_eq!(
        c.get_whois_server("test.ru").unwrap(),
        ROOT_WHOIS_SERVER.to_owned()
    );

    let mut c = Client::new();
    c.good_servers
        .insert("ru".to_owned(), "whois.ru".to_owned());
    c.good_servers
        .insert("edu.ru".to_owned(), "whois-test.edu.ru".to_owned());
    c.good_servers
        .insert("com".to_owned(), "whois-test.com".to_owned());
...
fn split_domain_test() {
    ...
    assert_eq!(split_domain(".test.ru."), vec!["test.ru", "ru", ""]);
    assert_eq!(
        split_domain("www.test.ru."),
        vec!["www.test.ru", "test.ru", "ru", ""]
    );
}
